...
class FCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        elif len(input_shape) == 3:  # [batch_size, n_coeffs, time_steps]
            # 輸入是3D（特徵序列），對時間維度進行平均
            logger.info(f"FCNN接收到3D輸入{input_shape}，對時間維度(dim=2)取平均")
            x = x.mean(dim=2)  # <-- 正確的維度：對時間維度進行平均
