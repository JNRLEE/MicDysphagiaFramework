...
class CustomClassificationLoader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 從數據文件讀取的動作類型映射 - 改為字典嵌套結構，保存每個資料夾的動作類型
        self.data_action_types = {}  # PatientID -> 動作類型
        self.directory_action_types = {}  # 目錄名 -> 動作類型
        self.patient_directory_map = {}  # PatientID -> 所有目錄列表
        self.directory_patient_map = {}  # 目錄名 -> PatientID
                ...
                try:
                    ...
                    if patient_id and selection:
                        ...
                        if selection_type:
                            ...
                            self.directory_action_types[dir_name] = selection_type
                            
                            # 記錄PatientID和動作類型的關係
                            self.data_action_types[patient_id] = selection_type
                            
                            # 建立PatientID與目錄的映射關係
                            if patient_id not in self.patient_directory_map:
                                self.patient_directory_map[patient_id] = []
                            self.patient_directory_map[patient_id].append(dir_name)
                            
                            # 建立目錄與PatientID的映射關係
                            self.directory_patient_map[dir_name] = patient_id
    ...
    def get_directory_action_type(self, directory_name: str) -> Optional[str]:
        """獲取特定目錄的動作類型
        ...
        Returns:
            Optional[str]: 動作類型名稱，若找不到則返回None
        """
        return self.directory_action_types.get(directory_name)
    
    def should_include_directory(self, directory_name: str) -> bool:
        """檢查指定目錄是否應該被包含在數據集中
        
        Description:
            根據目錄的動作類型和過濾設置，判斷該目錄是否應該被包含在數據集中。
            
        Args:
            directory_name (str): 目錄名稱
            
        Returns:
            bool: 如果目錄應該被包含，返回True；否則返回False
        """
        # 獲取目錄的動作類型
        action_type = self.get_directory_action_type(directory_name)
        if not action_type:
            return False
    ...
    def _load_excel_data(self) -> None:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                # 跳過空值
                if not patient_id or pd.isna(patient_id) or pd.isna(class_value) or class_value == 'nan':
                    continue
